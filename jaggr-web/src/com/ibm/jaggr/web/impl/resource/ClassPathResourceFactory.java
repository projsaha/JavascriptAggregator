...
public class ClassPathResourceFactory implements IResourceFactory {
	...
	public IResource newResource(URI uri) {
		...
		try {
			...
			String extractedResourceString = "";
			if ("classPath".equals(scheme)) { //$NON-NLS-1
				ClassLoader classLoader = Thread.currentThread().getContextClassLoader();
				if(uri.toString().indexOf("!") == -1){
					String uriPath = uri.getPath();					
					extractedResourceString = uriPath.startsWith("/") ? uriPath.substring(1) : uriPath;
				} else {
					extractedResourceString = uri.toString().substring(uri.toString().indexOf("!") + 2);					
				}
				...
				if(filePath != null){
					if (filePath.startsWith("file:") && filePath.contains("!")) {
						System.out.println("Inside classPathResourceFactory Start" + uri.toString());
						...
						System.out.println("Inside classPathResourceFactory End " + uri.toString());
					}
				} else{
